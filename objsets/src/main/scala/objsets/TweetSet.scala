...
object GoogleVsApple {
  ...
  /**
   * Methodology for google and apple tweets:
   * 1) Use TweetReader namespacce
   * 2) Return a set of all tweets
   * 3) filter all of the tweets whether there exists a word in google/apple in the tweet
   */
  
  lazy val googleTweets: TweetSet = TweetReader.
                                      allTweets.
                                      filter(x => google.exists(y => x.text.contains(y)))
  lazy val appleTweets: TweetSet = TweetReader.
                                      allTweets.
                                      filter(x => apple.exists(y => x.text.contains(y)))
